...
function ResumeBuilder({ onLogout, showNotice, notice }) {
  ...
  const handleExport = async () => {
    ...
    const canvas = await html2canvas(resumeElement, { scale: 2, useCORS: true, backgroundColor: '#fffef9' })
    ...
    const fileName = `${resume.firstName.trim()}${resume.lastName.trim()}CVcraft.pdf`.replace(/[^a-zA-Z0-9À-ÿ.-]/g, '')
    pdf.save(fileName)
